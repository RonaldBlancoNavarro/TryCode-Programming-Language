...
class MainFrame(Frame):
    ...
    def compilar(self):
        #print (self.txtInput.get(1.0, "end-1c"))
        text = self.txtInput.get(1.0, "end-1c")
        result, error = tryCode.run('<stdin>', text)

        if error: self.txtOutput.insert(1.0,error.as_string())  
        else: self.txtOutput.insert(1.0,result)    
